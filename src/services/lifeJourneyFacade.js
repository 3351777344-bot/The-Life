...
import { requestBackendScenario, requestBackendRoutes, requestBackendAdvisorReply, requestBackendRegretReport, requestBackendPlanningFileExtraction } from './backendService'
...
import { SERVICE_MODE, activeServiceMode, getServiceModeLabel } from './serviceConfig'
...
export const requestScenario = async (userInfo) => {
  if (isBackendApiMode) {
    try {
      ...
      return normalizeBackendResult(response, '场景生成失败，请稍后重试。', SERVICE_ERROR_CODE.SCENARIO_REQUEST_FAILED)
    } catch {
...
export const requestRoutes = async (userInfo, context) => {
  if (isBackendApiMode) {
    try {
      ...
      return normalizeBackendResult(response, '路线生成失败，请稍后重试。', SERVICE_ERROR_CODE.ROUTE_REQUEST_FAILED)
    } catch {
...
export const requestAdvisorReply = async (question, context) => {
  if (isBackendApiMode) {
    try {
      ...
      return normalizeBackendResult(response, '顾问暂时无法回复，请稍后重试。', SERVICE_ERROR_CODE.ADVISOR_REQUEST_FAILED)
    } catch {
...
export const requestRegretReport = async (payload) => {
  if (isBackendApiMode) {
    try {
      ...
      return normalizeBackendResult(response, '终局报告生成失败，请稍后重试。', SERVICE_ERROR_CODE.REPORT_REQUEST_FAILED)
    } catch {
...
export const requestPlanningFileExtraction = async (file) => {
  if (isBackendApiMode) {
    try {
      ...
      return normalizeBackendResult(response, '文件处理失败，请更换文件后重试。', SERVICE_ERROR_CODE.FILE_EXTRACTION_FAILED)
    } catch (error) {
